...
unsafe fn switch_process(process: &Process) 
{
	asm!(
		...
		in("a1") process.stack as *mut usize,
		in("a2") process.pc as *mut usize,
		options(noreturn),
	);
}
...
fn main() -> ! {
	...
	unsafe {
		let mut uart = uart::Uart::new();
		let mut process1 = Process::new(process1_func, /*0x80085000*/ (&_estack as *const u8 as usize) as *mut usize);
